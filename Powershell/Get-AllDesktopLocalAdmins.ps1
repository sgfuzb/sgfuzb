$AllDesktops = $null
$OutputDir = "c:\users\gills\desktop"
$LocalGroupName = "Administrators"
$OutputFile = Join-Path $OutputDir "LocalGroupMembers.csv"

Remove-Item $OutputFile # Reset file
...
$AllDesktops = Get-ADComputer -Filter * -Properties description | 
where { (($_.DistinguishedName -like "*CN=Computers*") -or ($_.DistinguishedName -like "*OU=Laptops*") ) -and ($_.enabled -eq $true) -and ($_.description  -notlike "*Failover*")} | 
Sort-Object Name 
#| Select-Object -First 5


ForEach($ComputerObj in $AllDesktops) {
        
        $computer = $ComputerObj.Name
