#
# Signed script (Check Adminsg cert is valid and updated in AD!) 
#

$WhatIfPreference = $False

# Paths
$RootDirectory1= "C:\Users\*\AppData\Local\Netcall"
#$RootDirectory2= "C:\Netcall"
$AddRemoveKey="HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall"
$UserKey= "HKU:\S-1-5-21*\Software\Netcall"
...
Remove-Item -recurse -force $RootDirectory1
#Remove-Item -recurse -force $RootDirectory2

# Find and remove all user specific chrome installs from the registry.
Get-ChildItem $UserKey -ErrorAction SilentlyContinue | Where-Object {($_.PSChildName -eq 'Netcall') } | Remove-Item -recurse -force
Get-ChildItem $AddRemoveKey -ErrorAction SilentlyContinue -recurse | Where-Object {
    ($_.PSChildName -eq '{E5B39E70-9206-4CCD-BF1F-9B4FAEFFDC85}') -or #5.4.3
	($_.PSChildName -eq '{40A1A3B3-46ED-4E0F-A24C-47004BF1D0E9}') -or #5.5.5
	($_.PSChildName -eq '{7177C3F5-CDF9-4DD9-AAD7-0747A24B630A}') -or #5.6.3
    ($_.PSChildName -eq '{9DCDB91B-2889-4841-B198-A41FA236F8F7}') -or #5.3.2
    ($_.PSChildName -eq '{C00A6715-1FFD-43A8-8BC6-5A3B4E64979E}') -or #5.2.3
    ($_.PSChildName -eq '{1E166E1A-65A3-4504-A18E-DCB005B87BB1}') -or #4.6
    ($_.PSChildName -eq '{95211BE6-D1B2-4776-B588-B61891F53850}') -or #4.13
    ($_.PSChildName -eq '{FAEF157F-ADE6-44B3-8044-9633037B0859}') -or #4.10
    ($_.PSChildName -eq '{A7B3A860-C374-4DD8-B486-EF8AF31A4398}') -or #3.29
    ($_.PSChildName -eq '{463F8B47-3834-4F65-A54E-589618E26DE7}') -or #3.2   
    ($_.PSChildName -eq '{512CDF87-017C-4376-A1E8-3A6D00810D6C}') -or #2.3+
    ($_.PSChildName -eq '{69480114-821D-4E8B-ADC9-F2ABD2D66241}') -or 
    ($_.PSChildName -eq '{0B717690-5CED-4975-A692-B13250AB8667}') -or 
    ($_.PSChildName -eq '{D9E8D8F9-62DE-4517-A5E6-72E3BC8F2FA8}')
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUOxO9aOuSz7hgwRxcEXnsL5tR
# QZ2gggyHMIIEnjCCA4agAwIBAgITKwAJd8is0KEeNtA2AwADAAl3yDANBgkqhkiG
...
# AYI3AgEVMCMGCSqGSIb3DQEJBDEWBBTeRqDB+cnrBHX2rtZu0sJrFLc1KjANBgkq
# hkiG9w0BAQEFAASBgI3/1rcwL0pPeuQAhnszhhfVJBrACRDKRGCG/s/HIb+Gr2eZ
# fKSZ+vGUellirmOVKbpEHOOv4jc6CRSsd/T6GZb6NdiuEEmUkGJkArH9SnyiicNq
# nvn11T0XfJynR9dzeW6HO81WuINezm+0Pxg4uLQnqcAqltGQtSbTPbWLahd+
