<#
    ...
    developed by Gleb Yourchenko, E-mail fnugry@null.net
    Version 1
...
# updates $serviceAccount table and removes completed job
    ...
    foreach( $job in $jobs ) {
        ...
        if ( $data.GetType() -eq [Object[]] ){
            $serviceList = $data | where-object { $_.StartName.toUpper().StartsWith( $currentDomain ) -or $_.StartName.toUpper().contains("@")}
